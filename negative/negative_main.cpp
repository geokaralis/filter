...
void AddMenus(HWND hwnd) {
	...
	AppendMenuW(hMenu, MF_STRING, IDR_MENU, L"&New");
	AppendMenuW(hMenu, MF_STRING, ID_FILE_OPEN, L"&Open");
	AppendMenuW(hMenu, MF_SEPARATOR, 0, NULL);
	AppendMenuW(hMenu, MF_STRING, ID_FILE_CLOSE, L"&Quit");

	AppendMenuW(hMenubar, MF_POPUP, (UINT_PTR)hMenu, L"&File");

	HMENU hMenu2 = CreateMenu();

	AppendMenuW(hMenu2, MF_STRING, ID_NEGATIVE, L"&Negative");
	AppendMenuW(hMenu2, MF_STRING, ID_APPLY, L"&Negative");
